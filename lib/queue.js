    ...
    try{
      ...
      _this.handler(job, function(err, data){
        if(err){
          ...
          completed(data);
		  if(job.data.rpc){
			  var rpcQ = Queue(job.data.rpc,_this.client.port, _this.client.host);
			  rpcQ.add({success: true})
		  }
        }
      });
    } catch(err){
...
Queue.prototype.rpc = function(data, options){

    var _this = this;

    return new Promise(function (resolve, reject) {
        var timer = setTimeout(function(){ failed(new Error('rpc response timeout')); },10 * 1000);
        var _uniq = Math.random().toString(36).substring(6);
        var rpcLabel = 'rpc-res:' + _uniq;
        var rpcQ = Queue(rpcLabel,_this.client.port, _this.client.host);

        rpcQ.on('failed',function(job,err){
            clearTimer(timer);
            failed(err);
        });

        rpcQ.process(function(job,done){
            clearTimeout(timer);
            done();
            rpcQ.delete();
            resolve(job.data);
        ...
        _this.add(data,options).catch(reject);
...
Queue.prototype.delete = function(){
    var _this = this;
    this.client.keys(this.toKey('*'),function(err,res){
        _.each(res,function(item){
            _this.client.del(item);
        })  
    })  
}   
